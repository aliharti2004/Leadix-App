            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="source"
                        ...
                        <option value="referral" {{ old('source', $lead->source) == 'referral' ? 'selected' : '' }}>Referral
                        </option>
                        <option value="cold_call" {{ old('source', $lead->source) == 'cold_call' ? 'selected' : '' }}>Cold
                            Call</option>
                        <option value="linkedin" {{ old('source', $lead->source) == 'linkedin' ? 'selected' : '' }}>LinkedIn
                        </option>
                ...
                <div>
                    ...
                    <select name="status"
                        ...
                        <option value="qualified" {{ old('status', $lead->status) == 'qualified' ? 'selected' : '' }}>
                            Qualified</option>
                        <option value="nurturing" {{ old('status', $lead->status) == 'nurturing' ? 'selected' : '' }}>
                            Nurturing</option>
                        <option value="disqualified" {{ old('status', $lead->status) == 'disqualified' ? 'selected' : '' }}>
                            Disqualified</option>
                    </select>
                    @error('status')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="estimated_value"
                            value="{{ old('estimated_value', $lead->estimated_value) }}" step="0.01" min="0"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" rows="4"
                        ...
                        placeholder="Add any additional notes or context about this lead...">{{ old('notes', $lead->notes) }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
