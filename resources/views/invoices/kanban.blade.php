<x-app-layout>
    <div x-data="invoiceKanbanApp()" class="flex flex-col min-h-[600px]">
        ...
        <div class="mb-6 flex-shrink-0"
            x-data="{ showFilters: true, hasActiveFilters: {{ request()->hasAny(['status', 'min_amount', 'max_amount', 'date_from', 'date_to', 'quick_date', 'company', 'sort_by', 'priority']) ? 'true' : 'false' }} }">
            ...
            <!-- Filter Panels -->
            <form action="{{ route('invoices.kanban') }}" method="GET" x-show="showFilters" x-transition>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                        ...
                        <div>
                            ...
                            <select name="company" onchange="this.form.submit()"
                                ...
                                @foreach(\App\Models\Lead::select('id', 'company')->distinct()->get() as $lead)
                                    <option value="{{ $lead->id }}" {{ request('company') == $lead->id ? 'selected' : '' }}>
                                        {{ $lead->company }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="priority" onchange="this.form.submit()"
                                ...
                                <option value="high" {{ request('priority') == 'high' ? 'selected' : '' }}>High ($10k+)
                                </option>
                        ...
                        <div>
                            ...
                            <select name="quick_date" onchange="this.form.submit()"
                                ...
                                <option value="due_today" {{ request('quick_date') == 'due_today' ? 'selected' : '' }}>Due
                                    Today</option>
                                <option value="due_week" {{ request('quick_date') == 'due_week' ? 'selected' : '' }}>Due
                                    This Week</option>
                                <option value="overdue" {{ request('quick_date') == 'overdue' ? 'selected' : '' }}>Overdue
                                </option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="sort_by" onchange="this.form.submit()"
                                ...
                                <option value="newest" {{ request('sort_by', 'newest') == 'newest' ? 'selected' : '' }}>
                                    Newest First</option>
                                <option value="oldest" {{ request('sort_by') == 'oldest' ? 'selected' : '' }}>Oldest First
                                </option>
                            </select>
                        </div>
                    </div>
        ...
        <div class="flex-1 overflow-hidden -mx-6">
            <div class="overflow-x-auto overflow-y-hidden px-6 pb-4">
                <div class="flex gap-5 pb-4">
                    @foreach(['draft' => 'Draft', 'pending' => 'Sent/Pending', 'paid' => 'Paid', 'overdue' => 'Overdue'] as $statusKey => $statusName)
                        <div class="flex-shrink-0 flex flex-col min-h-[500px]" style="width: 340px;">
                            <!-- Column Header -->
                            <div class="mb-4">
                                <div class="flex items-center justify-between mb-2">
                                    <span
                                        class="text-white font-bold text-sm uppercase tracking-wide">{{ $statusName }}</span>
                                    <span class="glass-card px-2 py-1 rounded-lg text-gray-400 text-xs font-bold">
                                        {{ $invoicesByStatus->get($statusKey)?->count() ?? 0 }}
                                    </span>
                                </div>
                                <p class="text-xs text-gray-600 font-semibold">
                                    ${{ number_format($invoicesByStatus->get($statusKey)?->sum('total') ?? 0, 0) }}
                                </p>
                            </div>

                            <!-- Invoice Cards -->
                            <div class="flex-1 overflow-y-auto space-y-3 pr-2" x-ref="status_{{ $statusKey }}"
                                ...
                                @forelse($invoicesByStatus->get($statusKey, collect()) as $invoice)
                                    <div class="invoice-card rounded-2xl p-4 cursor-pointer relative"
                                        data-invoice-id="{{ $invoice->id }}" @click="openDrawer({{ $invoice->id }})">

                                        <!-- Header: Invoice Number & Status -->
                                        <div class="flex items-start justify-between gap-2 mb-3">
                                            <div class="flex-1">
                                                <p class="text-gray-500 text-xs font-semibold mb-1">INVOICE</p>
                                                <h4 class="text-white text-base font-bold">{{ $invoice->invoice_number }}</h4>
                                            </div>
                                            <span class="px-2 py-0.5 rounded-md text-[10px] font-bold uppercase border
                                                        @if($statusKey === 'paid') bg-green-500/20 text-green-400 border-green-500/30
                                                        @elseif($statusKey === 'overdue') bg-red-500/20 text-red-400 border-red-500/30
                                                        @elseif($statusKey === 'pending') bg-amber-500/20 text-amber-400 border-amber-500/30
                                        ...
                                        <div class="space-y-2 mb-3">
                                            ...
                                            <div class="flex items-center justify-between text-xs">
                                                ...
                                                <span
                                                    class="text-gray-400 font-medium">{{ $invoice->issue_date ? $invoice->issue_date->format('M d, Y') : 'N/A' }}</span>
                                            </div>
                                            ...
                                            <div class="flex items-center justify-between text-xs">
                                                ...
                                                @if($invoice->due_date)
                                                    ...
                                                    @endphp
                                                    <span
                                                        class="font-bold {{ $isOverdue ? 'text-red-400' : ($isDueSoon ? 'text-amber-400' : 'text-gray-400') }}">
                                                        {{ $invoice->due_date->format('M d, Y') }}
                                                        @if($isOverdue)
                                                            ({{ abs($daysUntilDue) }}d overdue)
                                                        @elseif($isDueSoon && $invoice->status !== 'paid')
                                                            ({{ $daysUntilDue }}d left)
                                                        @endif
                                                    </span>
                                                @else
                                        ...
                                        <div class="pt-3 border-t border-white/5">
                                            <div class="flex items-center justify-between text-xs">
                                                <div class="flex items-center gap-1.5 text-gray-500">
                                                    ...
                                                    <span
                                                        class="font-semibold">{{ $invoice->created_at->diffForHumans() }}</span>
                                                </div>
        ...
        function invoiceKanbanApp() {
            return {
                ...
                init() {
                    ...
                    @foreach(['draft', 'pending', 'paid', 'overdue'] as $status)                     const status{{ ucfirst($status) }} = this.$refs.status_{{ $status }}; if (status{{ ucfirst($status) }}) { new Sortable(status{{ ucfirst($status) }}, { group: 'invoices', animation: 200, ghostClass: 'opacity-30', onEnd: (evt) => this.handleDrop(evt) }); }
                    @endforeach
                },
