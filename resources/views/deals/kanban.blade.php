<x-app-layout>
    <div x-data="kanbanApp()" class="flex flex-col min-h-[600px]">
        ...
        <div class="mb-6 flex-shrink-0"
            x-data="{ showFilters: true, hasActiveFilters: {{ request()->hasAny(['user_id', 'min_value', 'max_value', 'date_from', 'date_to', 'priority', 'deal_stage_id']) ? 'true' : 'false' }} }">
            ...
            <!-- Filter Panels -->
            <form action="{{ route('deals.kanban') }}" method="GET" x-show="showFilters"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <select name="deal_stage_id" onchange="this.form.submit()"
                                ...
                                @foreach($stages as $stage)
                                    <option value="{{ $stage->id }}" {{ request('deal_stage_id') == $stage->id ? 'selected' : '' }}>
                                        {{ $stage->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="priority" onchange="this.form.submit()"
                                ...
                                <option value="high" {{ request('priority') == 'high' ? 'selected' : '' }}>High ($50k+)
                                </option>
                        ...
                        <div>
                            ...
                            <select name="quick_date" onchange="this.form.submit()"
                                ...
                                <option value="week" {{ request('quick_date') == 'week' ? 'selected' : '' }}>This Week
                                </option>
                                <option value="month" {{ request('quick_date') == 'month' ? 'selected' : '' }}>This Month
                                </option>
                    ...
                    <div class="pt-4 border-t border-white/5">
                        <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                            ...
                            <div>
                                ...
                                <input type="number" name="min_value" value="{{ request('min_value') }}"
                                    placeholder="$0"
                        ...
                        <div class="flex-shrink-0 flex flex-col min-h-[500px]" style="width: 320px;">
                            ...
                            <div class="mb-4">
                                <div class="flex items-center justify-between mb-2">
                                    <span
                                        class="text-white font-bold text-sm uppercase tracking-wide">{{ $stage->name }}</span>
                                    <span class="glass-card px-2 py-1 rounded-lg text-gray-400 text-xs font-bold">
                                        {{ $dealsByStage->get($stage->id)?->count() ?? 0 }}
                                    </span>
                                </div>
                                <p class="text-xs text-gray-600 font-semibold">
                                    ${{ number_format($dealsByStage->get($stage->id)?->sum('value') ?? 0, 0) }}
                                </p>
                            </div>

                            <!-- Deal Cards - Scrollable -->
                            <div class="flex-1 overflow-y-auto space-y-3 pr-2" x-ref="stage_{{ $stage->id }}"
                                data-stage-id="{{ $stage->id }}"
                                style="scrollbar-width: thin; scrollbar-color: rgba(255,115,0,0.3) transparent;">

                                @forelse($dealsByStage->get($stage->id, collect()) as $deal)
                                    <div class="deal-card rounded-2xl p-4 cursor-pointer relative"
                                        data-deal-id="{{ $deal->id }}" @click="openDrawer({{ $deal->id }})">
                                        ...
                                        <div class="relative">
                                            ...
                                            <div class="flex items-start justify-between gap-2 mb-2">
                                                ...
                                                @endphp
                                                <span
                                                    class="px-2 py-0.5 rounded-md text-[10px] font-bold uppercase border {{ $priorityColors[$priority] }}">
                                                    {{ $priority }}
                                                </span>
                                            </div>
                                            ...
                                            <div class="mb-3 space-y-1">
                                                <div class="flex items-center gap-1.5 text-gray-400">
                                                    ...
                                                    <p class="text-xs font-semibold">{{ $deal->lead->company ?? 'No Company' }}
                                                    </p>
                                                </div>
                                                @if($deal->lead && ($deal->lead->first_name || $deal->lead->last_name))
                                                    <div class="flex items-center gap-1.5 text-gray-500">
                                                        ...
                                                        <p class="text-xs">{{ $deal->lead->first_name }}
                                                            {{ $deal->lead->last_name }}
                                                        </p>
                                                    </div>
                                                @endif
                                            ...
                                            <div class="flex flex-wrap gap-1.5 mb-3">
                                                ...
                                                @elseif($deal->expected_close_date && $deal->expected_close_date->isToday())
                                                    <span
                                                ...
                                                @elseif($deal->expected_close_date && $deal->expected_close_date->diffInDays(now()) <= 7)
                                                    <span
                            ...
                            <div>
                                ...
                                <select x-model="editForm.lead_id" required
                                    ...
                                    @foreach(\App\Models\Lead::all() as $lead)
                                        <option :value="{{ $lead->id }}" :selected="currentDeal.lead.id == {{ $lead->id }}">
                                            {{ $lead->company }}
                                        </option>
                                    @endforeach
                                </select>
    ...
    <script>
        function kanbanApp() {
            return {
                ...
                init() {
                    @foreach($stages as $stage)
                        const stage{{ $stage->id }} = this.$refs.stage_{{ $stage->id }};
                        if (stage{{ $stage->id }}) {
                            new Sortable(stage{{ $stage->id }}, {
                                group: 'deals',
                                animation: 200,
                ...
                openCreateModal() {
                    this.formData = {
                        ...
                        value: '',
                        user_id: '{{ auth()->id() }}',
                        deal_stage_id: '{{ isset($stages) && count($stages) > 0 ? $stages->first()->id : "" }}',
                        expected_close_date: ''
                    };
                    this.modalOpen = true;
                },
