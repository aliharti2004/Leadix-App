...
<?php $__env->startSection('header', 'Leads'); ?>
...
<?php $__env->startSection('content'); ?>
    ...
    <div>
        ...
        <div class="glass-card rounded-2xl p-5 border border-white/10 mb-6">
            <form method="GET" action="<?php echo e(route('leads.index')); ?>" class="grid grid-cols-1 md:grid-cols-5 gap-4">
                
                <div class="md:col-span-2">
                    <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Search leads..."
                ...
                <div>
                    <select name="status" onchange="this.form.submit()"
                        ...
                        <option value="nurturing" <?php echo e(request('status') == 'nurturing' ? 'selected' : ''); ?>>Nurturing</option>
                        <option value="disqualified" <?php echo e(request('status') == 'disqualified' ? 'selected' : ''); ?>>Disqualified
                        </option>
                ...
                <div>
                    <select name="source" onchange="this.form.submit()"
                        ...
                        <option value="event" <?php echo e(request('source') == 'event' ? 'selected' : ''); ?>>Event</option>
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $leads; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lead): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-white/5 transition">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div
                                        ...
                                        <?php echo e(strtoupper(substr($lead->contact_name ?? $lead->title, 0, 1))); ?>

                                    </div>
                                    <div>
                                        <p class="text-white font-bold text-sm"><?php echo e($lead->contact_name ?? $lead->title); ?></p>
                                        <p class="text-gray-500 text-xs"><?php echo e($lead->title); ?></p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 text-white text-sm"><?php echo e($lead->company ?? '-'); ?></td>
                            <td class="px-6 py-4 text-gray-400 text-sm"><?php echo e($lead->email ?? '-'); ?></td>
                            <td class="px-6 py-4">
                                <span class="px-3 py-1 rounded-full text-xs font-bold
                                        <?php if($lead->status == 'new'): ?> bg-blue-500/20 text-blue-400
                                        <?php elseif($lead->status == 'contacted'): ?> bg-purple-500/20 text-purple-400
                                        <?php elseif($lead->status == 'qualified'): ?> bg-green-500/20 text-green-400
                                        <?php elseif($lead->status == 'converted'): ?> bg-emerald-500/20 text-emerald-400
                                        <?php elseif($lead->status == 'disqualified'): ?> bg-red-500/20 text-red-400
                            ...
                            <td class="px-6 py-4 text-gray-400 text-sm capitalize">
                                <?php echo e(str_replace('_', ' ', $lead->source ?? '-')); ?></td>
                            <td class="px-6 py-4 text-white font-bold text-sm">
                                $<?php echo e(number_format($lead->estimated_value ?? 0, 0)); ?></td>
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <?php if($lead->status != 'converted'): ?>
                                        <form action="<?php echo e(route('leads.convert', $lead)); ?>" method="POST" class="inline">
                                            <?php echo csrf_field(); ?>
                                    ...
                                    <?php endif; ?>
                                    <a href="<?php echo e(route('leads.edit', $lead)); ?>" title="Edit"
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\dell\.gemini\antigravity\scratch\leadix-app\resources\views/leads/index.blade.php ENDPATH**/ ?>